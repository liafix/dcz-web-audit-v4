...
import { confirmReportAccessToken, findAuditById, markLeadNotificationSent, markLeadVerified } from '@/lib/db/queries';
...
export async function POST(request: Request, context: { params: Promise<{ token: string }> }) {
  try {
    ...
    const access = await confirmReportAccessToken(sha256(token));
    if (!access) throw new PublicAppError({ code: 'access_expired', status: 410, publicMessage: 'Odkaz už nie je platný. Pošlite si nový odkaz z výsledku.' });
    ...
    const lead = await markLeadVerified(audit.id, access.email);
    ...
    const scoredLead = lead ? await recalculateAndRouteLead(lead.id) : null;
    if (scoredLead) {
