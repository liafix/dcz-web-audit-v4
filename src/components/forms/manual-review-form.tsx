...
export function ManualReviewForm({ token }: { token: string }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4" noValidate>
      ...
      <Button type="submit" disabled={turnstile.pending || (Boolean(siteKey) && !turnstile.canSubmit)} className="w-full">{turnstile.pending ? 'Odosielame…' : 'Požiadať o manuálnu kontrolu'}</Button>
